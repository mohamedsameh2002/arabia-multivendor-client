...
export default function Header() {
  ...
  return (
    <header className={`flex fixed gap-2 pb-2 ${showHeader? 'top-0':'-top-44'} duration-200  bg-white z-10 flex-col w-full max-md:hidden`}>
        ...
        <div className='container flexBetween flex'>
          ...
          <div className='flex gap-5 items-center'>
            ...
            <div className='flex flexCenter'>
              <span className=' font-bold text-blackText'>تسجيل دخول</span>
              <RiUserLine className='text-blackText text-2xl hover:text-Red cursor-pointer' />
            </div>
            <span className='h-8 bg-gray-300 rounded-full w-px'></span>
            ...
            <span className='h-8 bg-gray-300 rounded-full w-px'></span>
            <button onClick={openSideCart} type='button' className="relative text-blackText text-2xl hover:text-Red cursor-pointer">
      ...
      <SideCart isCartOpen={isCartOpen} closeSideCart={closeSideCart}  ></SideCart>
    </header>
  )
}
